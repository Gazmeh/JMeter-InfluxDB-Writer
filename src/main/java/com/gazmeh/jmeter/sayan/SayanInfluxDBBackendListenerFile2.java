...
import com.gazmeh.jmeter.InfluxDBBackendListenerFile;
import com.gazmeh.jmeter.config.influxdb.RequestMeasurement;

/**
 * Backend listener that writes JMeter metrics to file.
 * 
 * The backend check for the sample type, and if it found a sayan response, then
 * convert it into a query and save into a file.
 * 
 * @author Alexander Wert
 * @author Mostafa Barmshory (mostafa.barmshory@gmail.com)
 *
 */
public class SayanInfluxDBBackendListenerFile2 extends InfluxDBBackendListenerFile {
    /**
     * Logger.
     */
    private static final Logger logger = LoggerFactory.getLogger(SayanInfluxDBBackendListenerFile2.class);

    private Map<String, Long> requestTimeStamps = new HashMap<>();

    public SayanInfluxDBBackendListenerFile2() {
	...
	if (systemTrackNumber != null) {
		...
		pointBuilder.addField("RequestTime", requestTime.longValue());
	    }
	}
